...
If the command doesn't report an LTS version of Node.js (e.g., v20.19.0), it means you don't have a suitable version of Node.js installed.
...
The search box uses https://pagefind.app[Pagefind] when the site has a Pagefind index, and otherwise the `/search` endpoint of the search engine container.
A site switches to Pagefind by indexing its pages after the Antora build:

 $ npx pagefind --site build/site

This writes the index to [.path]_build/site/pagefind/_, next to the UI in [.path]_build/site/_/_, and the search runs in the browser with no server.
The UI marks what gets indexed: the article body (`data-pagefind-body`), the page title, and the page version as metadata and a filter.
Navigation, the table of contents and the pagination links are left out.

`gulp preview` builds a Pagefind index of the preview pages too, so search works in the preview.

==== Search Analytics

With `site.keys.plausibleScript` set in the playbook, the UI loads https://plausible.io[Plausible] from the site's own domain and reports searches:

[source,yaml]
----
site:
  keys:
    plausibleScript: pa-XXXXXXXX.js # the site's script name from Plausible
----

* `Search`, with the normalized query (`query`) and the number of results (`results`).
  A search that ran after a pause in typing is reported only once the reader stays on its results for 2 seconds, clicks a result, or leaves the page.
  Partial words typed on the way to the final query are not reported.
* `Search Result Click`, with the query and the position of the opened result (`position`).

The browser never contacts plausible.io directly, so the site's nginx has to proxy both paths:

----
location = /js/pa-XXXXXXXX.js {
    proxy_pass https://plausible.io/js/pa-XXXXXXXX.js;
...
Browser tests in the [.path]_test/_ folder open the preview in headless Chromium and check that it renders: `icon:name[]` macros and admonition icons are visible, web fonts come from the site itself, and no third-party resources load beyond an allowlist.
...
The same tests also run against the assets from the packaged bundle, which are minified and have the CSS custom properties resolved, unlike the preview build:

 $ npm run test:bundle

The tests build and serve the preview themselves.
If `gulp preview` is already running, they reuse that server.
